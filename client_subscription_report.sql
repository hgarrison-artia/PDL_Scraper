select c.id,
       ...
       count(distinct d.id) as tracked_drug_count,
       string_agg(distinct d.name, ',') as tracked_drugs,
       count(distinct c2.id) as tracked_class_count,
       string_agg(distinct c2.name, ',') as tracked_classes
from clients c
    ...
    join client_tracked_drugs ctd on c.id = ctd.client_id
    join drugs d on ctd.drug_id = d.id
    join classifications c2 on d.classification_id = c2.id
group by c.id,
        cpf_bid_analysis.client_id,
        cpf_calendar.client_id,
        cpf_drug_info.client_id,
        cpf_notifications.client_id,
        cpf_state_reports.client_id,
        cpf_state_reports_premium.client_id,
        cpf_calendar_mobile.client_id,
        cpf_notifications_mobile.client_id,
        cpf_market_share_mobile.client_id,
        cpf_market_reports.client_id
order by id;
